...
- 카드뉴스 (레퍼런스 분석 프롬프트 + 1080x1350 HTML 템플릿 8장, 열어서 캡처하면 그림이 됩니다)
...
3. 받은 ZIP 파일의 압축을 풉니다. 맥은 파일을 더블클릭, 윈도우는 마우스 오른쪽 버튼 → **압축 풀기**. 풀면 `lecture-content-kit-main` 폴더가 생깁니다. 바탕화면이나 문서 폴더 등 찾기 쉬운 곳에 두세요.
...
1. AI 도구(Claude Code 또는 Codex)에서 압축 푼 폴더를 엽니다.
   - **Codex 앱** 새 작업을 시작하면 어느 폴더에서 일할지 고르는 화면이 나옵니다. 거기서 압축 푼 폴더를 고릅니다.
   - **Claude Code** 고르는 화면이 없습니다. 바로 아래 "이미 Claude Code를 쓰고 계신 분"을 보세요.
2. 이렇게 말합니다.
...
3. AI가 안내문을 읽었다고 답하면 폴더가 맞는 것입니다. 이어서 무엇을 만들지 물어봅니다. 고르면 해당 단계 안내대로 한 단계씩 확인받으며 진행합니다.
4. 내 결과물은 `내작업/` 폴더에 저장됩니다. 이 폴더는 깃에 올라가지 않습니다.

**이미 Claude Code를 쓰고 계신 분**

Claude Code는 켤 때 폴더를 고르는 화면이 없습니다. 검은 글씨 창에서 폴더를 먼저 정한 뒤 켭니다. 처음이시면 이 부분은 건너뛰고 Codex 앱을 쓰셔도 됩니다.

1. 검은 글씨 창을 엽니다. 맥은 **터미널** 앱, 윈도우는 **Git Bash**(윈도우에서 Git을 설치하면 같이 생기는 검은 창)입니다.
2. `cd`를 입력하고 빈칸을 하나 띄웁니다.
3. 압축 푼 폴더를 그 창 안으로 끌어다 놓습니다. 폴더 위치가 저절로 적힙니다. 엔터를 누릅니다.
4. `claude`를 입력하고 엔터를 누릅니다. 이제 그 폴더에서 Claude Code가 켜집니다. 위 2번 문장을 붙여 넣으면 됩니다.

**막히면**

- `사용량 한도(usage limit)` 메시지가 뜨면 요금제 한도입니다. 풀릴 때까지 방법 1로 진행하세요.
- 맥에서 `API usage limits` 오류가 나면 로그인 대신 개발자용 열쇠(환경변수 `ANTHROPIC_API_KEY`)가 먼저 쓰이고 있는 것입니다. 그 값을 지우고 다시 여세요.
- 윈도우에서 `claude` 또는 `codex`가 "이 시스템에서 스크립트를 실행할 수 없으므로"로 막히면, 윈도우가 외부 프로그램 실행을 막아 둔 것입니다. PowerShell을 열고 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`를 한 번 실행한 뒤 다시 시도하세요.
- Codex를 웹으로 연결해 주는 확장 앱(Codex Web GPT 등)을 깔았다면, Codex의 기본 설정이 웹 경유로 바뀌어 있을 수 있습니다. 이 안내문과 화면이 달라 보이면 그 앱을 끄거나 Codex 설정에서 기본 모델을 원래대로 돌린 뒤 다시 해 보세요.
...
| 카드뉴스를 이미지로 만들기 | Chrome. HTML을 열어 화면을 캡처하면 됩니다 (한 번에 뽑는 스크립트는 선택) |
| 장표·카드뉴스 보기 | 인터넷 연결 (글꼴을 인터넷에서 불러옵니다) |

따로 발급받을 열쇠(API 키)는 필요 없습니다. ChatGPT나 Claude에 로그인만 되어 있으면 됩니다.
...
| `AGENTS.md` · `CLAUDE.md` | AI가 이 폴더를 열었을 때 먼저 읽는 안내문 |
| `docs/` | README용 예시 이미지 |

기본 색은 아이보리 `#F7F4EC` · 검정 `#131313` · 코발트 `#0047AB`입니다. 장표와 카드뉴스 모두 색을 적어 둔 세 줄만 바꾸면 내 색이 됩니다.

## 사용한 오픈소스

이 키트는 아래 공개 자료의 규칙과 방법을 참고하거나 안내합니다. 고마운 분들입니다.

| 공개 자료 | 만든 곳 | 이 키트에서 쓰는 곳 |
|---|---|---|
...
"안내"로 표시한 것은 코드를 이 키트에 넣지 않고 링크로 소개만 했습니다. 쓸 때는 각 페이지의 라이선스와 안내를 따르세요.

## 저작권·면책

- 이 키트의 문서·템플릿·스크립트는 [MIT 라이선스](LICENSE)로 배포합니다. 저작권자: AIxSCHOOL
- 다른 분들이 만든 공개 자료와의 관계는 [NOTICE.md](NOTICE.md)에 적었습니다.
- 글꼴(Pretendard) 파일은 이 키트에 넣지 않고 인터넷에서 불러옵니다.
